...
namespace JayTom.Dws.Client.ViewModels.Pages.Preferences.LogsViewModel
{
    ...
    {
        private readonly ILogQueryService<OutputLogInfoModel> _logQueryService;
        ...
        private ObservableCollection<OutputLogItemModel> _outputLogItems = new();
        private SnackbarMessageQueue _outputLogMessageQueue = new(TimeSpan.FromSeconds(2));

        public OutputLogPageViewModel(ILogQueryService<OutputLogInfoModel> logQueryService)
        {
            _logQueryService = logQueryService;
        }

        public SnackbarMessageQueue OutputLogMessageQueue
        {
            get => _outputLogMessageQueue;
            set => SetProperty(ref _outputLogMessageQueue, value);
        }

        public ObservableCollection<OutputLogItemModel> OutputLogItems
        {
            get => _outputLogItems;
            set => SetProperty(ref _outputLogItems, value);
        }
        ...
        {
            get => new DelegateCommand<OutputLogItemModel>(ClickDelegate);
        }

        private async void ClickDelegate(OutputLogItemModel obj)
        {
            //显示详细信息
        ...
        private async void OpenDateTimeDialogDelegate(object obj)
        {
            await UiThread.Dispatcher.InvokeAsyncUnwrapped(async () =>
            {
                ...
                {
                    model.Identifier = "OutputLogDialog";
        ...
        private async void LoadData(int pageIndex)
        {
            ...
            await UiThread.Dispatcher.InvokeAsyncUnwrapped(async () =>
            {
                ...
                if (loadingDialog.DataContext is LoadingDialogViewModel model)
                {
                    model.Identifier = "OutputLogDialog";
                    DialogHost.Show(loadingDialog, model.Identifier)
                        .Forget("显示输出日志加载进度对话框");
                    await Task.Delay(500);
                    OutputLogItems.Clear();
                    Details = string.Empty;
                    var result = await _logQueryService.SearchAsync(
                        new LogQuery(StartTime, EndTime, SelectLogType, Content: OutputContent),
                        pageIndex - 1,
                        pageSize);
                    if (result.Total > 0)
                    {
                        ...
                        {
                            var cameraLogItemModels = result.Items.Select(s => new OutputLogItemModel()
                            {
                            ...
                            OutputLogItems.AddRange(cameraLogItemModels);
                        }
                        else
                        {
                            OutputLogMessageQueue?.Enqueue("Error loading data. Please try again.");
                        }
                    }
                    else
                    {
                        OutputLogMessageQueue?.Enqueue("No data matching the criteria found.");
                    }
