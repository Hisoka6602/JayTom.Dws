...
namespace JayTom.Dws.Infrastructure.Repository.LocalLog {
    ...
    public class CleanupLogRepository : LocalRepositoryBase<LogCleaningLogInfoModel, SqliteLogsContext>, ICleanupLogRepository {

        public CleanupLogRepository(IDbContextFactory<SqliteLogsContext> contextFactory, IMemoryCache cache) : base(contextFactory, cache) {
        }

        public async Task<KeyValuePair<bool, string>> DeleteDataThanDays(int days) {
            try {
                await using var concardContext = _contextFactory.CreateDbContext();
                var dbSet = concardContext?.Set<LogCleaningLogInfoModel>();
