...
namespace JayTom.Dws.Infrastructure.Repository.LocalLog {
    ...
    public class ExceptionLogRepository : LocalRepositoryBase<ExceptionLogInfoModel, SqliteLogsContext>, IExceptionLogRepository {

        public ExceptionLogRepository(IDbContextFactory<SqliteLogsContext> contextFactory, IMemoryCache cache) : base(contextFactory, cache) {
        }

        public async Task<KeyValuePair<bool, string>> DeleteDataThanDays(int days) {
            try {
                await using var concardContext = _contextFactory.CreateDbContext();
                var dbSet = concardContext?.Set<ExceptionLogInfoModel>();
