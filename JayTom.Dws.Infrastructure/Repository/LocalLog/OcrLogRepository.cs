...
namespace JayTom.Dws.Infrastructure.Repository.LocalLog {
    ...
    public class OcrLogRepository : LocalRepositoryBase<OcrLogInfoModel, SqliteLogsContext>, IOcrLogRepository {

        public OcrLogRepository(IDbContextFactory<SqliteLogsContext> contextFactory, IMemoryCache cache) : base(contextFactory, cache) {
        }

        public async Task<KeyValuePair<bool, string>> DeleteDataThanDays(int days) {
            try {
                await using var concardContext = _contextFactory.CreateDbContext();
                var dbSet = concardContext?.Set<OcrLogInfoModel>();
