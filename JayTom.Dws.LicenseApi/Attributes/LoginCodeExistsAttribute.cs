...
namespace JayTom.Dws.LicenseApi.Attributes {
    ...
    public class LoginCodeExistsAttribute : ValidationAttribute {
        ...
        protected override ValidationResult? IsValid(object? value, ValidationContext validationContext) {
            if (value is not null) {
                ...
                if (licenseUserRepository is not null) {
                    ...
                    var licenseUserInfo = licenseUserInfos?.FirstOrDefault(f =>
                        (f.Phone.Equals(value.ToString()) ||
                        f.UserName.Equals(value.ToString()) ||
                        f.UserCode.Equals(value.ToString())));
