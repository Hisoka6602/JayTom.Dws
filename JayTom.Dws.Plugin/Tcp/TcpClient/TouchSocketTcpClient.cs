...
namespace JayTom.Dws.Plugin.Tcp.TcpClient {
    ...
    public class TouchSocketTcpClient : ITcpCommClient {
        ...
        /// 自动重连同步锁。
        /// </summary>
        private readonly System.Threading.Lock _reconnectSync = new();
        ...
        /// 保护客户端分帧接收缓冲区。
        /// </summary>
        private readonly System.Threading.Lock _receiveSync = new();
        /// <summary>
        /// 保存尚未组成完整固定长度帧的接收字节。
        /// </summary>
        ...
        /// <summary>
        /// 将 TouchSocket 的收包线程与协议解析、业务事件、日志和 UI 完全隔离。
        /// 无界通道用于设备指令，满载时不能丢包，也不能反向阻塞网络回调。
        /// </summary>
        private readonly Channel<(byte[] Frame, DateTime ReceivedTime)> _receivedFrames =
            Channel.CreateUnbounded<(byte[] Frame, DateTime ReceivedTime)>(new UnboundedChannelOptions {
                SingleReader = true,
                SingleWriter = false,
                AllowSynchronousContinuations = false
            });
        /// <summary>严格按收到顺序发布完整帧的专用消费者。</summary>
        ...
        /// <summary>创建 TCP 客户端并启动专用收帧消费者。</summary>
        public TouchSocketTcpClient() {
            ...
            try {
                ...
                if (_tcpClient is null) {
                    ...
                    _tcpClient.Received += delegate (TouchSocket.Sockets.TcpClient client, ByteBlock block, IRequestInfo info) {
                        try {
                            HandleReceivedBytes(block.Buffer, block.Len, DateTime.Now);
                            block.Clear();
                        }
                        catch (Exception e) {
        ...
        /// <summary>
        /// 处理 TCP 粘包和拆包，并发布完整消息帧。
        /// </summary>
        private void HandleReceivedBytes(byte[] buffer, int length, DateTime receivedTime) {
            if (length <= 0) {
                return;
            }
            ...
            lock (_receiveSync) {
                if (DataLen <= 0) {
                    ...
                    for (var index = 0; index < length; index++) {
                        _receiveBuffer.Add(buffer[index]);
                    }

                    while (_receiveBuffer.Count - _receiveBufferOffset >= DataLen) {
                        var frame = new byte[DataLen];
                        _receiveBuffer.CopyTo(_receiveBufferOffset, frame, 0, DataLen);
                        _receiveBufferOffset += DataLen;
                        frames.Add(frame);
                    }

                    if (_receiveBufferOffset == _receiveBuffer.Count) {
                        _receiveBuffer.Clear();
                        _receiveBufferOffset = 0;
                    }
                    else if (_receiveBufferOffset >= 4096) {
                        _receiveBuffer.RemoveRange(0, _receiveBufferOffset);
                        _receiveBufferOffset = 0;
                    }
                }
            }
            ...
            foreach (var frame in frames) {
                if (!_receivedFrames.Writer.TryWrite((frame, receivedTime))) {
                    OnException(new InvalidOperationException("TCP接收帧队列已停止，完整报文未能入队"));
                }
            }
        }

        /// <summary>
        /// 在专用消费者上执行字符串格式化和所有通信订阅者，网络收包线程不会运行任何业务代码。
        /// </summary>
