                ...
                using (var httpClient = _httpClientFactory.CreateClient("INSURANCE")) {
                    ...
                    await using (Stream dataStream = new MemoryStream(Encoding.UTF8.GetBytes(requestJson))) {
                        using (HttpContent content = new StreamContent(dataStream)) {
                            ...
                            message = await httpClient.PostAsync($"{Domain}{"/api/BarCode/BarcodeInfos"}", content, cancellationToken)
        ...
        public async Task<KeyValuePair<bool, object>> GroupedNodeNames(CancellationToken cancellationToken = default) {
            try {
                using (var httpClient = _httpClientFactory.CreateClient("INSURANCE")) {
                    ...
                    HttpResponseMessage message;
                    message = await httpClient.GetAsync($"{Domain}{"/api/BarCode/GroupedNodeNames"}", cancellationToken)
                        .ConfigureAwait(false);
                    string httpResult;
                    switch (message.StatusCode) {
                        case HttpStatusCode.OK: {
